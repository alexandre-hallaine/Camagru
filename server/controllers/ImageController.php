...
class ImageController
{
    ...
    public function handleImages(Request $request): void
    {
        ...
        if ($requestMethod === "GET") {
            ...
            $likedImages = isset($_SESSION["id"])
                ? $this->likeModel->getLikedImagesByUser($_SESSION["id"])
                : [];
    ...
    public function deleteImage(Request $request, int $id): void
    {
        $requestMethod = $request->getMethod();
    ...
    public function likeImage(Request $request, int $id): void
    {
        $requestMethod = $request->getMethod();
    ...
    public function commentImage(Request $request, int $id): void
    {
        $requestMethod = $request->getMethod();
        if ($requestMethod !== "POST") {
            sendResponse(405, ["message" => "Method Not Allowed"]);
        ...
        $imageOwnerId = $this->imageModel->getUserIdByImageId($id);
        $settings = $this->settingModel->findByUserId($imageOwnerId);
    ...
    private function getAllUsers(): array
    {
        ...
        $stmt->execute();
        $users = $stmt->fetchAll();
        return array_combine(array_map(fn($u) => $u["id"], $users), $users);
    }
}
