...
class AuthController
{
    ...
    public function register(): void
    {
        ...
        $userId = $this->userModel->create($input["username"], password_hash($input["password"], PASSWORD_DEFAULT));
    ...
    public function reset(): void
    {
        ...
        action($this->pdo, $user["id"], "RESET_PASSWORD", ["password" => password_hash($input["password"], PASSWORD_DEFAULT)]);
    }
    ...
    public function token(): void
    {
        ...
        if ($action["kind"] === "VERIFY_ACCOUNT") {
            ...
            $this->userModel->updatePassword($action["user_id"], $payload["password"]);
        } elseif ($action["kind"] === "CHANGE_EMAIL") {
            $this->settingModel->updateEmail($action["user_id"], $payload["email"]);
        }
